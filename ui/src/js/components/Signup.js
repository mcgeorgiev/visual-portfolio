  ...
  render() {
    return (
      <div className='form landing-form'>
        ...
        <form className='form'>
          <input type="text" name="fullname" onChange={this.handleChange({eventHandler: this.props.signupFullNameChanged})} placeholder='Full Name'/>
          <input type="text" name="email" onChange={this.handleChange({eventHandler: this.props.signupEmailChanged})} placeholder='Email'/>
          <img src={this.state.eyeIcon} onClick={this.toggleShowPassword} className='show-password-icon'/>
          <input type={this.state.showPassword} name="password" onChange={this.handleChange({eventHandler: this.props.signupPasswordChanged})} placeholder='Password'/>
          <button type="button" className='signup-button' onClick={this.props.signupSubmitted}> SIGN ME UP</button>
